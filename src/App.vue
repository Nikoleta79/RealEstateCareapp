...
export default {
      ...
      if (store.getters.offlineInspections && store.getters.offlineInspections.length > 0) {
        try {
          ...
          store.getters.offlineInspections.forEach(inspection => {
            ...
            response.data.record.addresses[addressIndex].inspections[inspectionIndex].cleanlinessScore = inspection.cleanlinessScore; // CleanlinessScore
  ...
  watch: {
    // Watch for changes in the dark theme state and update the body class
    'store.getters.darkTheme': function(newValue) {
